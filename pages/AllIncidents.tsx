...
export const AllIncidents: React.FC<AllIncidentsProps> = ({ onViewDetail }) => {
  const [currentPage, setCurrentPage] = useState(() => {
    const saved = sessionStorage.getItem("adani_incidents_page");
    return saved ? parseInt(saved) : 1;
  });
  const [activeTab, setActiveTab] = useState<string>(() => {
    return sessionStorage.getItem("adani_incidents_tab") || "ALL";
  });
  const [searchQuery, setSearchQuery] = useState(() => {
    return sessionStorage.getItem("adani_incidents_search") || "";
  });
  ...
  const [loading, setLoading] = useState(
    apiData.length === 0 && totalCount === 0,
  );
  const [isPolling, setIsPolling] = useState(true);
  const [confidenceFilter, setConfidenceFilter] = useState<number>(() => {
    const saved = sessionStorage.getItem("adani_incidents_confidence");
    return saved ? parseInt(saved) : 0;
  });

  // Sync state changes to sessionStorage
  useEffect(() => {
    sessionStorage.setItem("adani_incidents_page", currentPage.toString());
  }, [currentPage]);

  useEffect(() => {
    sessionStorage.setItem("adani_incidents_tab", activeTab);
  }, [activeTab]);

  useEffect(() => {
    sessionStorage.setItem("adani_incidents_search", searchQuery);
  }, [searchQuery]);

  useEffect(() => {
    sessionStorage.setItem("adani_incidents_confidence", confidenceFilter.toString());
  }, [confidenceFilter]);
  ...
  const fetchIncidents = useCallback(async () => {
    try {
      const response = await fetch(
        `https://isafetyrobo.binarysemantics.org/api/violations?page=1&limit=${ITEMS_PER_PAGE}`,
      );
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      fetchIncidents();
    }, 20000); // 5s is better for "socket" feel while being polite to the API
  ...
  const filteredData = useMemo(() => {
    return apiData.filter((inc) => {
      ...
      const matchesSearch =
        inc.event_id.toLowerCase().includes(searchQuery.toLowerCase()) ||
        inc.metadata.label.toLowerCase().includes(searchQuery.toLowerCase()) ||
        inc.metadata.zone_camera
          .toLowerCase()
          .includes(searchQuery.toLowerCase());
      const confidenceValue = Number(inc.metadata?.confidence || 0) * 100;
      const matchesConfidence = confidenceFilter === 0 || confidenceValue >= confidenceFilter;
      return matchesStatus && matchesSearch && matchesConfidence;
    });
  }, [apiData, activeTab, searchQuery, confidenceFilter]);
  ...
  const getStatusIcon = (status: string) => {
    ...
    if (s === "PENDING")
      return <div className="w-1.5 h-1.5 bg-orange-500 rounded-full"></div>;
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm">
            Real-time log of AI-detected safety events from the vision API.
          </p>
        </div>
        ...
        >
          ...
          <div>
            ...
            <h4 className="text-2xl font-black text-red-900 leading-none">
              {
                apiData.filter(
                  (i) => i.metadata.status.toUpperCase() === "ACTIVE",
                ).length
              }
            </h4>
          </div>
        ...
        >
          ...
          <div>
            ...
            <h4 className="text-2xl font-black text-orange-900 leading-none">
              {
                apiData.filter(
                  (i) => i.metadata.status.toUpperCase() === "PENDING",
                ).length
              }
            </h4>
          </div>
        ...
        >
          ...
          <div>
            ...
            <h4 className="text-2xl font-black text-green-900 leading-none">
              {
                apiData.filter(
                  (i) => i.metadata.status.toUpperCase() === "RESOLVED",
                ).length
              }
            </h4>
          </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden min-h-[500px] flex flex-col">
          ...
          <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
            <div className="relative">
              ...
              <input
                ...
                className="pl-9 pr-4 py-2 border border-slate-200 rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-primary w-full sm:w-72 transition-all"
              />
            </div>
            <select
              ...
              className="px-4 py-2 border border-slate-200 rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-primary text-slate-700 font-medium transition-all cursor-pointer"
            >
        ...
        <div
          ref={scrollRef}
          className="overflow-y-auto flex-1 max-h-[600px] no-scrollbar scroll-smooth"
        >
              ...
              {loading && apiData.length === 0 ? (
                ...
                filteredData.map((inc, index) => (
                  <tr
                    ...
                    onClick={() => onViewDetail?.(inc.id.toString())}
                  >
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`px-2 py-0.5 rounded text-[10px] font-semibold uppercase tracking-wider ${getSeverityStyles(inc.metadata.severity)}`}
                      >
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-col">
                        <span className="text-[14px] text-slate-700 font-semibold max-w-[50px]">
                          {Number(inc.metadata?.confidence) * 100} %
                        </span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
