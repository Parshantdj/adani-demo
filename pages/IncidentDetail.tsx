  ...
  React.useEffect(() => {
    ...
    if (saved) {
      ...
      const found = allIncidents.find(
        (inc: any) => inc.id?.toString() === incidentId || inc.event_id === incidentId,
      );
      if (found) {
        setIncident(found);
      }
  ...
  const displaySteps = BASE_PPE_LIFECYCLE_STEPS.map((step) => {
    if (isResolved) {
      ...
      return {
        ...
        status: "completed",
        timestamp:
          step.timestamp === "Ongoing" || step.timestamp === "-"
            ? "10:25:12 AM"
            : step.timestamp,
        description:
          step.id === 4
            ? "Corrective action verified and worker compliance confirmed."
            : step.id === 5
              ? "System re-verified PPE status. Incident closed."
              : step.description,
      };
    }
    return step;
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-[10px] font-mono font-semibold text-primary">
                {incident.event_id.substring(0, 8)}
              </span>
            </div>
            <h2 className="text-2xl font-semibold text-slate-900">
              {incident.metadata?.label}
            </h2>
          </div>
        </div>
          ...
          <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
            <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
              ...
              <span className="text-[10px] font-mono text-slate-400">
                STREAM: {incident.stream_id.substring(0, 8)}
              </span>
            </div>
            <div
            ...
            >
              <img
                src={
                  incident.s3_url ||
                  "https://picsum.photos/seed/incident/800/600"
                }
              ...
              <div className="absolute bottom-4 left-4 right-4 flex justify-between items-center bg-black/40 backdrop-blur-md p-2 rounded-lg border border-white/10 opacity-0 group-hover:opacity-100 transition-opacity">
                <span className="text-[10px] text-white font-semibold tracking-widest font-mono">
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="p-3 bg-slate-50 rounded-2xl border border-slate-100">
                  ...
                  <span
                    className={`px-2 py-0.5 rounded text-[10px] font-semibold uppercase ${incident.metadata.severity.toUpperCase() === "CRITICAL" ? "bg-red-600 text-white" : "bg-orange-500 text-white"}`}
                  >
                ...
                <div className="p-3 bg-slate-50 rounded-2xl border border-slate-100">
                  ...
                  <div className="flex items-center gap-1.5">
                    <div
                      className={`w-1.5 h-1.5 rounded-full ${isResolved ? "bg-green-500" : "bg-red-500 animate-pulse"}`}
                    ></div>
                    <span className="text-[10px] font-semibold text-slate-900 uppercase tracking-widest">
          ...
          <div className="bg-primary p-6 rounded-3xl text-white shadow-xl shadow-primary-100 relative overflow-hidden">
            ...
            <ul className="space-y-3">
              {[
                ...
                <li
                  key={idx}
                  className="flex gap-2 text-xs font-medium text-primary-50 leading-relaxed"
                >
          ...
          <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm">
            ...
            <div className="relative pl-12 space-y-12 pb-4">
              ...
              <div className="absolute left-6 top-2 bottom-6 w-0.5 bg-slate-100">
                <div
                  className={`absolute top-0 left-0 w-full bg-indigo-500 rounded-full shadow-[0_0_10px_rgba(99,102,241,0.5)] transition-all duration-1000`}
                  style={{ height: isResolved ? "100%" : "60%" }}
                ></div>
              </div>
              ...
              {displaySteps.map((step) => (
                <div
                  key={step.id}
                  className={`relative group ${step.status === "pending" ? "opacity-40 grayscale" : ""}`}
                >
                  {/* Step Marker */}
                  <div
                    className={`absolute -left-12 w-12 h-12 flex items-center justify-center rounded-full border-4 border-white shadow-md z-10 transition-all ${step.status === "completed"
                        ? "bg-green-500 text-white"
                        : step.status === "active"
                          ? "bg-indigo-600 text-white scale-110 shadow-indigo-100 animate-pulse"
                  ...
                  {/* Step Content Card */}
                  <div
                    className={`p-6 rounded-2xl border transition-all ${step.status === "active"
                  ...
                  >
                    <div className="flex items-center justify-between mb-4">
                      <div>
                        <h4
                          className={`text-lg font-semibold ${step.status === "active" ? "text-indigo-900" : "text-slate-900"}`}
                        >
                          {step.title}
                        </h4>
                      ...
                      <span
                        className={`px-3 py-1 rounded-full text-[10px] font-semibold uppercase tracking-widest ${step.status === "completed"
                        ...
                        {step.details.map((detail, idx) => (
                          ...
                          >
                            ...
                            <p className="text-xs font-semibold text-slate-700">
                              {detail.key === "Confidence Score"
                                ? incident.metadata?.confidence * 100 + "%"
                                : detail.value}
                            </p>
                          </div>
                        ))}
      ...
      {isEnlarged && (
        ...
        >
          ...
          <div className="relative w-full max-w-6xl aspect-video bg-slate-900 rounded-2xl overflow-hidden shadow-2xl border border-white/5 animate-in zoom-in-95 duration-300">
            <img
              src={
                incident.s3_url || "https://picsum.photos/seed/incident/800/600"
              }
